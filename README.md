...
- When Telegram calls that webhook with a new message, the handler replies
  with `sendMessage(chat_id, "yes")` — attached to an inline keyboard with
  three buttons ("👍 Yes", "🤔 Maybe", "👎 No") — and returns immediately.
- Tapping any of those buttons fires a `callback_query` update; the handler
  answers it with a "yes" toast and sends another "yes" (buttons included),
  because the answer's always yes no matter which one you pick.
